...
async function doRefund(row: OpsBooking) {
  const reason = window.prompt('请输入退款原因', '商家预约退款');
  if (reason === null) return;
  try {
    await refundOrder(row.booking.orderId, reason.trim() || '商家预约退款');
        ...
        <tbody>
          <tr v-for="row in bookings" :key="row.booking.orderId">
            ...
            <td>
              ...
              >{{ row.booking.storeConfirmStatus === 'confirmed' ? '已确认' : row.booking.storeConfirmStatus === 'cancelled' ? '已取消' : '确认预约' }}</button>
              <button
                v-if="canRefund(row)"
                class="text-btn"
                @click="doRefund(row)"
