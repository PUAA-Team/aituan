...
onMounted(() => {
  ...
  window.addEventListener('aituan-admin-auth-expired', handleAuthExpired);
});

onUnmounted(() => {
  window.removeEventListener('aituan-admin-auth-expired', handleAuthExpired);
});
...
<template>
  ...
  >
    ...
    <AdminProfilePage v-else-if="activePage === 'adminProfile'" :refresh-key="refreshKey" @notice="setNotice" />
