...
class CouponSelectorPage extends StatefulWidget {
  const CouponSelectorPage({super.key, required this.args});

  final CouponSelectorArgs args;
...
class _CouponSelectorPageState extends State<CouponSelectorPage> {
          ...
          AppCard(
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text(
                    '当前订单金额 ¥${widget.args.orderAmount.toStringAsFixed(2)}，选择优惠券后将重新试算优惠与实付金额。',
                    style: Theme.of(context).textTheme.bodySmall,
                  ),
                ),
                if (widget.args.selectedCoupon != null)
                  TextButton(
  ...
  void _select(OrderCouponOption coupon) {
    ...
    Navigator.pop(context, CouponSelectorResult.selected(coupon));
  }
}
...
class _CouponOptionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AppCard(
    ...
    backgroundColor: coupon.usable ? null : AppColors.soft,
    child: Row(
      children: [
        Container(
          width: 72,
