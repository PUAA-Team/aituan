...
class BackendAppRepository {
  ...
  Future<List<MessageItem>> fetchMessages({String? type}) async {
    final typeQuery = type == null || type.isEmpty
        ? ''
        : 'type=${Uri.encodeQueryComponent(type)}&';
    final json = await _get(
      '/api/app/message/station?${typeQuery}page=1&pageSize=20',
    );
    final page = _map(json['data']);
    return _list(
      page['list'],
  ...
  Future<void> markMessageRead(String messageId) async {
    await _patch(
      '/api/app/message/station/$messageId/read',
      <String, dynamic>{},
    );
  }

  Future<void> markAllMessagesRead() async {
    await _patch('/api/app/message/station/read-all', <String, dynamic>{});
  }
...
// 解析助手统一委托到 core/network/json_codec.dart，保持既有调用点不变。
List<String> _strings(dynamic value) => jsonStringList(value);

Map<String, dynamic> _map(dynamic value) => jsonMap(value);

List<dynamic> _list(dynamic value) => jsonList(value);

String _string(dynamic value, {String fallback = ''}) =>
    jsonString(value, fallback: fallback);

String? _nullableString(dynamic value) => jsonStringOrNull(value);

int _int(dynamic value) => jsonInt(value);

int? _nullableInt(dynamic value) => jsonIntOrNull(value);

double _double(dynamic value) => jsonDouble(value);

double? _nullableDouble(dynamic value) => jsonDoubleOrNull(value);

bool _bool(dynamic value) => jsonBool(value);

DateTime? _dateTime(dynamic value) => jsonDateTime(value);
