...
import com.aituan.engagementplatform.client.MemberGrowthClient;
...
class InteractionService {
  ...
  ReviewView submitReview(long orderId, ReviewCreateRequest request) {
    ...
    PlatformRemoteClient.ReviewEligibility eligibility = remoteClient.reviewEligibility(orderId);
    PlatformRemoteClient.OrderSnapshot snapshot = eligibility.order();
    if (snapshot.userId() != userId) throw new BusinessException(ErrorCode.NOT_FOUND);
    if (!eligibility.eligible()) {
      throw new BusinessException(ErrorCode.ORDER_STATE_INVALID,
          eligibility.reason() == null ? "订单完成后才能评价" : eligibility.reason());
    }
    PlatformRemoteClient.UserSnapshot user = remoteClient.userSummary(userId);
    InteractionRepository.OrderReviewRow order = new InteractionRepository.OrderReviewRow(
        snapshot.orderId(), snapshot.orderNo(), snapshot.title(), snapshot.storeId(),
        snapshot.merchantId(), snapshot.storeName(), user.nickname());
    return interactionRepository.findReviewByOrder(userId, orderId)
        .map(row -> toUserReviewView(row, userId))
        .orElseGet(() -> {
          ...
          try {
            remoteClient.markOrderReviewed(orderId, reviewId);
            interactionRepository.markOrderSyncSucceeded(reviewId);
          } catch (BusinessException exception) {
            interactionRepository.markOrderSyncFailed(reviewId, exception.getMessage());
          }
          try {
            memberGrowthClient.addReviewGrowth(userId, reviewId);
          } catch (BusinessException ignored) {
            // 成长值属于非关键副作用，失败不回滚已发布评价，由幂等键支持后续补偿。
          }
