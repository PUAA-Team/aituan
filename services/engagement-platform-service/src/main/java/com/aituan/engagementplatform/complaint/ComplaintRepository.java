...
class ComplaintRepository {
  ...
  private static final String TICKET_SELECT = """
      select t.id, t.ticket_no, t.user_id, t.order_id, t.store_id, t.merchant_id,
             ...
             t.order_no, t.store_name, t.user_nickname
  ...
  private void appendAdminFilters(
    ...
    if (orderNoFilter != null) { sql.append(" and t.order_no = ?"); args.add(orderNoFilter); }
    if (storeNameFilter != null) { sql.append(" and t.store_name like ?"); args.add("%" + storeNameFilter + "%"); }
  }
  ...
  Long insertTicket(String ticketNo, long userId, Long orderId, Long storeId, Long merchantId,
                    String orderNo, String storeName, String userNickname,
    ...
    jdbcTemplate.update(
        ...
        insert into complaint_ticket(ticket_no, user_id, order_id, store_id, merchant_id,
                                     order_no, store_name, user_nickname,
                                     category, title, detail, evidence_urls, status)
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
        """,
        ticketNo, userId, orderId, storeId, merchantId, orderNo, storeName, userNickname,
        category, title, detail, evidenceUrls);
