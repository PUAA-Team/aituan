...
public class TradeRepository {
  ...
  public Long insertOrder(OrderInsertRow order) {
    jdbcTemplate.update(
        """
        insert into order_main(order_no, user_id, store_id, merchant_id, coupon_id, store_name, order_type, title, display_status, payment_status,
        ...
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        order.orderNo(), order.userId(), order.storeId(), order.merchantId(), order.couponId(), order.storeName(), order.orderType(), order.title(),
  ...
  public void insertOrderItem(long orderId, OrderItemInsertRow item) {
    jdbcTemplate.update(
        """
        insert into order_item(order_id, item_id, sku_id, item_name, item_subtitle, business_type, category_id, quantity, unit_price, total_price, cover_url)
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        orderId, item.itemId(), item.skuId(), item.itemName(), item.subtitle(), item.businessType(), item.categoryId(), item.quantity(),
  ...
  public boolean markOrderReviewed(long orderId, long reviewId) {
    int updated = jdbcTemplate.update(
        "update order_main set review_id = ?, updated_at = current_timestamp where id = ? and is_deleted = 0 and (review_id is null or review_id = ?)",
        reviewId, orderId, reviewId);
  ...
  public long countCompletedOrders(long userId) {
    Long count = jdbcTemplate.queryForObject(
        "select count(1) from order_main where user_id = ? and payment_status = 'paid' and refund_status = 'none' and display_status = 'used' and is_deleted = 0",
        Long.class, userId);
    return count == null ? 0 : count;
  }

  public List<Long> topStoreIds(long userId, int limit) {
    return jdbcTemplate.query(
        "select store_id from order_main where user_id = ? and is_deleted = 0 group by store_id order by count(1) desc, store_id limit ?",
        (rs, rowNum) -> rs.getLong("store_id"), userId, limit);
  }

  public List<Long> topCategoryIds(long userId, int limit) {
    return jdbcTemplate.query(
        "select i.category_id from order_item i join order_main o on o.id = i.order_id where o.user_id = ? and o.is_deleted = 0 and i.is_deleted = 0 and i.category_id is not null group by i.category_id order by count(1) desc, i.category_id limit ?",
        (rs, rowNum) -> rs.getLong("category_id"), userId, limit);
  }

  public StoreOrderMetricsRow storeOrderMetrics(long storeId) {
    return jdbcTemplate.queryForObject(
        "select count(1) order_count, coalesce(sum(payable_amount), 0) amount, sum(case when display_status in ('unpaid','pending') then 1 else 0 end) pending_count from order_main where store_id = ? and is_deleted = 0",
        (rs, rowNum) -> new StoreOrderMetricsRow(rs.getLong("order_count"), rs.getBigDecimal("amount"), rs.getLong("pending_count")), storeId);
  }

  public PlatformOrderMetricsRow platformOrderMetrics() {
    return jdbcTemplate.queryForObject(
        "select count(1) order_count, sum(case when payment_status = 'paid' then 1 else 0 end) paid_order_count, sum(case when refund_status <> 'none' then 1 else 0 end) refunded_order_count, coalesce(sum(case when payment_status = 'paid' then payable_amount else 0 end), 0) total_amount from order_main where is_deleted = 0",
        (rs, rowNum) -> new PlatformOrderMetricsRow(rs.getLong("order_count"), rs.getLong("paid_order_count"), rs.getLong("refunded_order_count"), rs.getBigDecimal("total_amount")));
  }

  public List<OrderItemRow> listOrderItems(long orderId) {
    return jdbcTemplate.query(
        """
        select id, order_id, item_id, sku_id, item_name, item_subtitle, business_type, category_id, quantity, unit_price, total_price, cover_url, is_reviewed
  ...
  public void cancelBookingForRefund(long orderId, String reason) {
    jdbcTemplate.update(
        """
        update order_booking_record
        set store_confirm_status = 'cancelled',
  ...
  private List<OrderRow> queryOrders(String where, Object... args) {
    return jdbcTemplate.query(
        """
        select id, order_no, user_id, store_id, merchant_id, coupon_id, review_id, store_name, order_type, title, display_status, payment_status, fulfillment_status,
  ...
  private OrderRow mapOrder(ResultSet rs, int rowNum) throws SQLException {
    return new OrderRow(
        rs.getLong("id"), rs.getString("order_no"), rs.getLong("user_id"), rs.getLong("store_id"), nullableLong(rs, "merchant_id"),
        nullableLong(rs, "coupon_id"), nullableLong(rs, "review_id"), rs.getString("store_name"),
  ...
  private OrderItemRow mapOrderItem(ResultSet rs, int rowNum) throws SQLException {
    return new OrderItemRow(rs.getLong("id"), rs.getLong("order_id"), rs.getLong("item_id"), nullableLong(rs, "sku_id"), rs.getString("item_name"),
  ...
  public record OrderItemInsertRow(Long itemId, Long skuId, String itemName, String subtitle, String businessType, Long categoryId, int quantity,
                                   BigDecimal unitPrice, BigDecimal totalPrice, String coverUrl) {}

  public record OrderRow(Long id, String orderNo, Long userId, Long storeId, Long merchantId, Long couponId, Long reviewId,
                         String storeName, String orderType, String title,
  ...
  public record OrderItemRow(Long id, Long orderId, Long itemId, Long skuId, String itemName, String itemSubtitle, String businessType, Long categoryId,
                             int quantity, BigDecimal unitPrice, BigDecimal totalPrice, String coverUrl, boolean isReviewed) {}

  public record StoreOrderMetricsRow(long orderCount, BigDecimal amount, long pendingCount) {}

  public record PlatformOrderMetricsRow(long orderCount, long paidOrderCount, long refundedOrderCount, BigDecimal totalAmount) {}
