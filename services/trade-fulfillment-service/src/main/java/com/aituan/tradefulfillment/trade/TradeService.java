...
import com.aituan.tradefulfillment.trade.client.InventoryClient.InventoryLine;
...
public class TradeService {
  ...
  public OrderDetailView createOrder(CreateOrderRequest request) {
    ...
    List<InventoryLine> inventoryLines = inventoryLines(context.items());

    Long orderId = tradeRepository.insertOrder(new OrderInsertRow(
        userId,
        context.store().id(),
        context.store().merchantId(),
        request.couponId(),
    ...
    try {
      if (request.couponId() != null) {
        couponClient.use(request.couponId(), userId, orderId, preview.amount());
      }
      return getOrderDetail(orderId);
    } catch (RuntimeException exception) {
      if (request.couponId() != null) {
        try {
          couponClient.release(request.couponId(), orderId);
        } catch (RuntimeException compensationFailure) {
          exception.addSuppressed(compensationFailure);
        }
      }
      try {
        inventoryClient.restore(orderId, inventoryLines);
      } catch (RuntimeException compensationFailure) {
        exception.addSuppressed(compensationFailure);
      }
      throw exception;
  ...
  private void refundStaffOrder(OrderRow order, String reasonText) {
    ...
    releaseOrderResources(order);
    memberGrowthClient.refundOrderGrowth(order.userId(), order.id(), order.payableAmount());
  ...
  private void releaseOrderResources(OrderRow order) {
    inventoryClient.restore(order.id(), inventoryLinesFromRows(tradeRepository.listOrderItems(order.id())));
    if (order.couponId() != null) {
      couponClient.release(order.couponId(), order.id());
    }
  }
  ...
  private List<InventoryLine> inventoryLines(List<TradeItem> items) {
    return items.stream().map(item -> {
      if (item.item().skuId() == null) {
        throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "商品缺少库存 SKU 快照");
      }
      return new InventoryLine(item.item().skuId(), item.item().id(), item.quantity());
    }).toList();
  }

  private List<InventoryLine> inventoryLinesFromRows(List<OrderItemRow> items) {
    return items.stream().map(item -> {
      if (item.skuId() == null) {
        throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "订单缺少库存 SKU 快照，无法恢复库存");
      }
      return new InventoryLine(item.skuId(), item.itemId(), item.quantity());
    }).toList();
  }
  ...
  private OrderItemInsertRow toOrderItemInsert(TradeItem item) {
    ...
    return new OrderItemInsertRow(item.item().id(), item.item().skuId(), item.item().itemName(), item.item().subtitle(), item.item().businessType(), item.item().categoryId(),
        item.quantity(), money(valueOrZero(item.item().price())), total, item.item().coverUrl());
  }
