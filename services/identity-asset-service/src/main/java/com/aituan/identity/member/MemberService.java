...
import com.aituan.identity.coupon.CouponService;
...
public class MemberService {
  ...
  MemberService(MemberRepository memberRepository, ObjectMapper objectMapper, CouponService couponService, IdentityAuditClient identityAuditClient) {
  ...
  MemberLevelView createLevel(MemberLevelUpsertRequest request) {
    ...
    identityAuditClient.publish(actorId, "member_level_create", "member_level", id, "新增会员等级:" + request.levelName());
  ...
  MemberLevelView updateLevel(long id, MemberLevelUpsertRequest request) {
    ...
    identityAuditClient.publish(actorId, "member_level_update", "member_level", id, "更新会员等级:" + request.levelName());
    return memberRepository.findLevel(id).map(this::toView)
        .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND));
  }
