...
class InternalIdentityService {
  ...
  InternalIdentityService(InternalIdentityRepository repository, PasswordEncoder passwordEncoder) {
  ...
  UserPreferenceSignalsView preferenceSignals(long userId) {
    repository.findUserSummary(userId).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND));
    return new UserPreferenceSignalsView(userId, repository.listPreferenceSignals(userId));
  }
  ...
  MerchantAccountProvisionView provisionMerchantAccount(MerchantAccountProvisionRequest request, String idempotencyKey) {
    ...
    return repository.findAccountByLogin(loginName)
        .map(row -> new MerchantAccountProvisionView(row.accountId(), row.accountNo(), row.loginName(), row.status()))
        .orElseGet(() -> {
          long accountId = repository.insertMerchantAccount(loginName, request.phone(), request.email(), passwordEncoder.encode(request.password()));
          InternalIdentityRepository.AccountRow created = repository.findAccountByLogin(loginName).orElseThrow();
          return new MerchantAccountProvisionView(accountId, created.accountNo(), created.loginName(), created.status());
        });
  }
