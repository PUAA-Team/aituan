...
class InternalIdentityRepository {
  ...
  List<FavoriteSignal> listPreferenceSignals(long userId) {
    return jdbcTemplate.query(
        ...
        limit 50
        """,
        (rs, rowNum) -> new FavoriteSignal(rs.getString("favorite_type"), rs.getLong("target_id"), rs.getString("target_name")),
        userId);
  }
  ...
  private AddressSnapshotView mapAddressSnapshot(ResultSet rs, int rowNum) throws SQLException {
    return new AddressSnapshotView(rs.getLong("id"), rs.getLong("user_id"), rs.getString("contact_name"), rs.getString("contact_phone"),
        ...
        rs.getBigDecimal("latitude") == null ? null : rs.getBigDecimal("latitude").doubleValue(), rs.getString("delivery_note"));
  }
