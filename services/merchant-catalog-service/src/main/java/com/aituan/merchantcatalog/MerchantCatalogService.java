...
class MerchantCatalogService {
  ...
  @Transactional
  InventoryResultView deductInventory(InventoryDeductRequest request, String callerService, String idempotencyKey) {
    requireIdempotencyKey(idempotencyKey);
    return executeInventoryIdempotently("deduct", request.orderId(), idempotencyKey, callerService, () -> {
  ...
  @Transactional
  InventoryResultView restoreInventory(InventoryRestoreRequest request, String callerService, String idempotencyKey) {
    requireIdempotencyKey(idempotencyKey);
    return executeInventoryIdempotently("restore", request.orderId(), idempotencyKey, callerService, () -> {
      List<InventoryLineResultView> lines = new ArrayList<>();
      for (InventoryItemRequest item : request.items()) {
        MerchantCatalogRepository.SkuRow sku = repository.findSku(item.skuId()).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND));
        if (!sku.itemId().equals(item.itemId())) throw new BusinessException(ErrorCode.BAD_REQUEST, "SKU 与商品不匹配");
  ...
  private InventoryResultView executeInventoryIdempotently(String action, long orderId, String idempotencyKey, String callerService, Supplier<InventoryResultView> operation) {
    String caller = callerService == null || callerService.isBlank() ? "unknown" : callerService.trim();
    MerchantCatalogRepository.InventoryIdempotencyRow existing = repository.findInventoryIdempotency(caller, action, idempotencyKey).orElse(null);
    if (existing != null) return deserializeResult(idempotencyKey, existing.status(), existing.resultSummary());

    InventoryResultView result = operation.get();
    repository.insertInventoryIdempotency(caller, action, idempotencyKey, serializeRequest(orderId), serializeResult(result.lines()), result.status());
    return result;
  }

  private String serializeRequest(long orderId) {
    return "orderId=" + orderId;
  }

  private String serializeResult(List<InventoryLineResultView> lines) {
    try {
      return objectMapper.writeValueAsString(lines);
    } catch (JsonProcessingException exception) {
      throw new BusinessException(ErrorCode.INTERNAL_ERROR, "库存幂等结果序列化失败");
    }
  }

  private InventoryResultView deserializeResult(String idempotencyKey, String status, String resultSummary) {
    try {
      List<InventoryLineResultView> lines = objectMapper.readValue(resultSummary, objectMapper.getTypeFactory().constructCollectionType(List.class, InventoryLineResultView.class));
      return new InventoryResultView(idempotencyKey, status, lines);
    } catch (JsonProcessingException exception) {
      throw new BusinessException(ErrorCode.INTERNAL_ERROR, "库存幂等结果解析失败");
    }
  }
