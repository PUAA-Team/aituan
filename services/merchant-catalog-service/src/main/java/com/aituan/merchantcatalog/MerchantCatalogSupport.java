...
class FileStorageService {
  ...
  FileStorageService(@Value("${aituan.file.public-prefix:/api/common/files}") String publicPrefix) {
    this.publicPrefix = publicPrefix == null || publicPrefix.isBlank() ? "/api/common/files" : publicPrefix;
  }

  FileAssetView save(MultipartFile file, String bizType) {
    String original = file == null || file.getOriginalFilename() == null ? "upload.bin" : file.getOriginalFilename();
    String safeName = original.replaceAll("[^a-zA-Z0-9._-]", "_");
    String type = bizType == null || bizType.isBlank() ? "merchant" : bizType.trim();
    String url = publicPrefix + "/" + type + "/" + System.currentTimeMillis() + "-" + safeName;
    return new FileAssetView(url);
  }
...
class InternalServiceClient {
  ...
  private <T> T get(String url, ParameterizedTypeReference<T> dataType) {
    try {
      ResponseEntity<InternalApiResponse<T>> response = restTemplate.exchange(url, HttpMethod.GET, new HttpEntity<>(headers(null)), new ParameterizedTypeReference<InternalApiResponse<T>>() {});
      InternalApiResponse<T> body = response.getBody();
      return body == null || body.code() != 0 ? null : body.data();
  ...
  private <T> T post(String url, Object request, String idempotencyKey, ParameterizedTypeReference<T> dataType) {
    try {
      ResponseEntity<InternalApiResponse<T>> response = restTemplate.exchange(url, HttpMethod.POST, new HttpEntity<>(request, headers(idempotencyKey)), new ParameterizedTypeReference<InternalApiResponse<T>>() {});
      InternalApiResponse<T> body = response.getBody();
      return body == null || body.code() != 0 ? null : body.data();
    } catch (RestClientException ex) {
      return null;
